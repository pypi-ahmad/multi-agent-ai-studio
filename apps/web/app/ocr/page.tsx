...
export default function OcrPage() {
  const [prompt, setPrompt] = useState("Extract text fields and confidence checks from scanned invoice pages.");
  ...
  const run = async () => {
    try {
      ...
      const chat = await apiRequest<{ id: string }>({ path: "/chat", method: "POST", token, body: { title: "OCR Workspace" } });
      const response = await apiRequest<ChatResponse>({
        path: `/chat/${chat.id}/messages`,
        method: "POST",
        token,
        body: { content: `OCR Agent Task: ${prompt}`, context: { workspace: "ocr" } },
      });
  ...
  return (
    <PageFrame title="OCR" description="OCR agent planning for scanned docs, screenshots, and handwritten content.">
      ...
      <article className="rounded-xl border border-border bg-card p-4 text-sm space-y-3">
        ...
        <button type="button" onClick={run} className="rounded bg-accent px-4 py-2 text-white">Run OCR Agent</button>
      </article>
